# stats_utils.py — light/stub version
...
def compute_kpis(df: pd.DataFrame | None, start_capital: float = 10_000.0) -> Dict[str, Any]:
    if df is None or len(df) == 0:
        return {"trades": 0, "wins": 0, "losses": 0, "winrate": 0.0,
                "pnl_total": 0.0, "fees_total": 0.0, "avg_rr": 0.0,
                "avg_roi": 0.0, "account_value": start_capital}
    ...
    return {"trades": trades, "wins": wins, "losses": losses, "winrate": winrate,
            "pnl_total": pnl_total, "fees_total": fees_total,
            "avg_rr": avg_rr, "avg_roi": avg_roi, "account_value": account_value}

def render_kpi_bar(st, df: pd.DataFrame, cfg: dict | None = None) -> None:
    start_capital = 10_000.0
    if cfg:
        sc = (cfg.get("START_CAPITAL") or cfg.get("start_capital")
              or (cfg.get("budget") or {}).get("start_capital"))
        try:
            if sc is not None:
    ...
    cols[5].metric("Account", f"{k['account_value']:.2f}")
