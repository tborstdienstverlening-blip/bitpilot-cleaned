...
def render_kpi_bar(start_btc: float, kpi: dict) -> None:
    ...
    # Rij 1
    # Startkapitaal (BTC): nu VASTE 8 decimalen
    r1[0].metric("Startkapitaal (BTC)", format_btc(start_btc, decimals=8, fixed=True))

    r1[1].metric(
        "Actueel kapitaal (BTC)",
        format_btc(kpi["acct_now"]),  # blijft legacy (trim), geen wijziging gevraagd
    ...
    r2[0].metric(
        "ROI %",
        format_pct_signed(kpi["roi_pct"]),
        delta=(format_pct_signed(kpi.get("last_roi_pp"))
               if kpi.get("last_roi_pp") is not None else None)
    )
    last_win_add = kpi.get("last_win_add")
    last_loss_add = kpi.get("last_loss_add")
    r2[1].metric("Winnende trades", f"{kpi['wins']}",
                 delta=(f"+{last_win_add}" if last_win_add is not None else None))
    r2[2].metric("Verloren trades", f"{kpi['losses']}",
                 delta=(f"+{last_loss_add}" if last_loss_add is not None else None))

    last_result_text = "—" if kpi.get("last_result") is None else f"Laatst: {kpi['last_result']}"
    winrate_pct = kpi.get("winrate_pct")
    r2[3].metric("Winrate %",
                 ("—" if winrate_pct is None else f"{winrate_pct:.2f}%"),
                 delta=last_result_text)
