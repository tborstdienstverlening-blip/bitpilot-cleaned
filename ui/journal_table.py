...
from services.pnl_service import to_decimal
...
from utils.formatters import format_btc_fixed8
...
def map_row_to_payload(r: pd.Series, base_row: dict) -> dict:
    ...
    # PNL invoer: EU -> Decimal -> float; leeg -> 0.0
    for k in (COL["PNL_TP1"], COL["PNL_TP2"], COL["PNL_TP3"], COL["PNL_EXIT"]):
        d = to_decimal(r.get(k))
        out[k] = 0.0 if d is None else float(d)
    ...
    out[COL["RR_ACTUAL"]] = r.get(COL["RR_ACTUAL"], base_row.get(COL["RR_ACTUAL"], ""))  # afgeleid
...
def render_table(df_view: pd.DataFrame) -> None:
    """
    Journal-tabel (read-only en edit-mode) incl. CRUD/ghost-row.
    Kolomvolgorde/labels/kleur exact gelijk; PNL-weergave 8 dec, geen sci-notatie.
    """
    vis_cols = [
        COL["DATUM"],
        "ID",
    ...
    # 🔧 PNL_TP1/2/3 en PNL_Exit als strings in editor voor EU-komma
    for c in [COL["PNL_TP1"], COL["PNL_TP2"], COL["PNL_TP3"], COL["PNL_EXIT"]]:
        if c in df_view.columns:
            df_view[c] = pd.Series(df_view[c]).astype("string").fillna("")

    # Editor-config: TextColumn (EU-acceptatie) + help; PNL_BTC read-only
    colcfg = {
        ...
        # PnL invoer als tekst (EU-komma), 8-dec rendering gebeurt in read-only
        ...
        "PNL_BTC": st.column_config.NumberColumn("PNL_BTC", help="TP1+TP2+TP3+Exit (BTC)", disabled=True, format="%.8f", step=0.00000001),
    ...
    if edit_mode:
        ...
        # Read-only view met 8-decimalen en kleur
        ...
        styler = (
            ...
            .format({
                COL["PNL_TP1"]: lambda x: "" if pd.isna(x) else format_btc_fixed8(x),
                COL["PNL_TP2"]: lambda x: "" if pd.isna(x) else format_btc_fixed8(x),
                COL["PNL_TP3"]: lambda x: "" if pd.isna(x) else format_btc_fixed8(x),
                COL["PNL_EXIT"]: lambda x: "" if pd.isna(x) else format_btc_fixed8(x),
                "PNL_BTC":     lambda x: "" if pd.isna(x) else format_btc_fixed8(x),
            })
        )
    ...
    if do_save:
        ...
        df_ed = df_view[vis_cols].reset_index(drop=True)  # we halen wijzigingen uit widget state via df_view
        ...
        if ok_all:
            msg_bits = []
            if appended: msg_bits.append(f"{appended} toegevoegd")
            if updated: msg_bits.append(f"{updated} gewijzigd")
            if skipped_empty: msg_bits.append(f"{skipped_empty} leeg overgeslagen")
